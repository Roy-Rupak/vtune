...
void vtss_cpuevents_reqcfg_default(int need_clear, int defsav)
{
    ...
    for (i = 0; i < CPU_EVENTS_SUPPORTED; i++) {
        if (i > 1 && hardcfg.family != VTSS_FAM_P6)
            break;
        ...
        DEBUG_CPUEVT("[%02d]: %s: into mux_grp=%d", reqcfg.cpuevent_count_v1, cpuevent_desc[i].name, mux_cnt);
        /// copy event name
...
void vtss_sysevents_reqcfg_append(void)
{
    ...
    for (i = sys_event_idx; i < vtss_sysevent_end && reqcfg.cpuevent_count_v1 < VTSS_CFG_CHAIN_SIZE; i++) {
        ...
        for (j = 0; j <= mux_grp && reqcfg.cpuevent_count_v1 < VTSS_CFG_CHAIN_SIZE; j++) {
            ...
            DEBUG_CPUEVT("[%02d]: %s: into mux_grp=%d of %d", reqcfg.cpuevent_count_v1, sysevent_desc[i].name, j, mux_grp);
            /// copy event name
...
static void vtss_cpuevents_validate_event(cpuevent_t *event, char *name)
{
    ...
    {
        ERROR("%s: Bad event selection MSR: 0x%x", name, event->selmsr);
    ...
    if ((event->cntmsr < VTSS_IA32_PMC0       || event->cntmsr >= VTSS_IA32_PMC0       + pmu_counter_no) &&
        (event->cntmsr < VTSS_IA32_FIXED_CTR0 || event->cntmsr >= VTSS_IA32_FIXED_CTR0 + pmu_fixed_counter_no))
    {
        ERROR("%s: Bad counter MSR: 0x%x", name, event->cntmsr);
        event->selmsr = VTSS_IA32_PERFEVTSEL0;
        event->selmsk = 0;
        event->cntmsr = VTSS_IA32_PMC0;
    }
    ...
    {
        ERROR("%s: Bad extra event MSR: 0x%x", name, event->extmsr);
        event->extmsr = 0;
        event->extmsk = 0;
    }
}
...
void vtss_cpuevents_upload(cpuevent_t *cpuevent_chain, cpuevent_cfg_v1_t *cpuevent_cfg, int count)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        if (cpuevent_cfg[i].event_id >= VTSS_CFG_CHAIN_SIZE) {
            ...
            /// fixup fixed mask w/o PMI flag (APEBS configuration)
            if (cpuevent_cfg[i].selmsr.idx == VTSS_IA32_FIXED_CTR_CTRL) {
                int offset;
                for (offset = 0; offset < 16; offset += 4) {
                    if (((cpuevent_cfg[i].selmsr.val >> offset) & 0xf) == 0x3)
                        cpuevent_cfg[i].selmsr.val |= 0x8 << offset; //set "enable PMI on overflow" bit
                }
            }
            /// copy MSRs
            cpuevent_chain[i].selmsr = cpuevent_cfg[i].selmsr.idx;
            cpuevent_chain[i].selmsk = cpuevent_cfg[i].selmsr.val; //clear APEBS flags by converting 64 bits to 32
            ...
            /// adjust sampling intervals if no sampling was requested (but we still wrap the counters around)
            if (!cpuevent_chain[i].interval && !cpuevent_cfg[i].cntmsr.val) {
                cpuevent_cfg[i].interval = cpuevent_chain[i].interval = CPU_EVTCNT_THRESHOLD;
            }
            /// set up counter offset for fixed events
            if (cpuevent_cfg[i].selmsr.idx == VTSS_IA32_FIXED_CTR_CTRL) {
                /// form the modifier to enable correct masking of control MSR in vft->restart()
                int cnto = cpuevent_cfg[i].cntmsr.idx - VTSS_IA32_FIXED_CTR0;
                cpuevent_chain[i].modifier = (unsigned int)((cpuevent_cfg[i].selmsr.val >> (4*cnto)) << 16);
                ((event_modifier_t*)&cpuevent_chain[i].modifier)->cnto = cnto;
            } else {
                cpuevent_chain[i].modifier = (int)(cpuevent_cfg[i].selmsr.val & VTSS_EVMOD_ALL);
            }
            if (cpuevent_cfg[i].selmsr.idx == VTSS_IA32_FIXED_CTR_CTRL) {
                if (cpuevent_cfg[i].cntmsr.idx != VTSS_IA32_FIXED_CTR0+2 && pmu_fixed_counter_no > 2) {
        ...
        DEBUG_CPUEVT("[%02d]: %s: si=%d, mod=%x, selmsr=%x, cntmsr=%x, selmsk=%x", i,
            ((char*)&cpuevent_cfg[i] + cpuevent_cfg[i].name_off), cpuevent_chain[i].interval,
            cpuevent_chain[i].modifier, cpuevent_chain[i].selmsr, cpuevent_chain[i].cntmsr,
            cpuevent_chain[i].selmsk);
    }
    if (i) {
...
void vtss_cpuevents_sample(cpuevent_t *cpuevent_chain)
{
    ...
    for (i = 0; i < VTSS_CFG_CHAIN_SIZE && cpuevent_chain[i].valid; i++) {
        DEBUG_CPUEVT("[%02d]: mux_idx=%d, mux_grp=%d of %d %s", i,
...
void vtss_cpuevents_quantum_border(cpuevent_t *cpuevent_chain, int flag)
{
    ...
    for (i = 0; i < VTSS_CFG_CHAIN_SIZE && cpuevent_chain[i].valid; i++) {
        DEBUG_CPUEVT("[%02d]: mux_idx=%d, mux_grp=%d of %d %s flag=%d", i,
...
int vtss_cpuevents_init_pmu(int defsav)
{
    ...
    if (reqcfg.cpuevent_count_v1 == 0 && !(reqcfg.trace_cfg.trace_flags & (VTSS_CFGTRACE_CTX|VTSS_CFGTRACE_PWRACT|VTSS_CFGTRACE_PWRIDLE))) {
        ...
        REPORT("There is no configuration was get from runtool, so init defaults");
        vtss_cpuevents_reqcfg_default(1, defsav);
        vtss_sysevents_reqcfg_append();
    }
    on_each_cpu(vtss_cpuevents_save,  NULL, 1);
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 2, 0)
...
void vtss_cpuevents_fini_pmu(void)
{
    ...
    on_each_cpu(vtss_cpuevents_stop_all, NULL, 1);
    on_each_cpu(vtss_cpuevents_restore,  NULL, 1);
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 2, 0)
...
int vtss_cpuevents_init(void)
{
    ...
    REPORT("PMU: fixed counters: %d, general counters: %d", pmu_fixed_counter_no, pmu_counter_no);
    ...
    if (hardcfg.family == VTSS_FAM_P6) {
        ...
        cpuevent_desc[2].name     = "CPU_CLK_UNHALTED.REF_TSC";
        cpuevent_desc[2].desc     = "CPU_CLK_UNHALTED.REF_TSC";
    ...
    if (hardcfg.family == VTSS_FAM_P6) {
        if (hardcfg.model == VTSS_CPU_SNB ||
           ...
           hardcfg.model == VTSS_CPU_HSW_M ||
           hardcfg.model == VTSS_CPU_HSW_G)
        {
