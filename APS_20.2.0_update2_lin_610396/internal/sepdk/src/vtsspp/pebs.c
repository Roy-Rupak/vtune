...
int vtss_pebs_version(void)
{
    vtss_pebs_t *pebs;

    if (vtss_pebs_record_size == sizeof(pebs->v3)) return 3;
    else if (vtss_pebs_record_size == sizeof(pebs->v2)) return 2;
    else if (vtss_pebs_record_size == sizeof(pebs->v1)) return 1;
    return -1;
}
...
static void vtss_pebs_release_buffer(int cpu)
{
    ...
    cea = pcb(cpu).pebs_ptr;
    vtss_cea_clear(cea, PEBS_BUFFER_SIZE);
    buffer = pcb(cpu).pebs_vaddr;
...
static int vtss_pebs_alloc_buffer(int cpu)
{
    ...
    pcb(cpu).pebs_ptr = NULL;
    buffer = kmalloc_node(PEBS_COUNT*sizeof(vtss_pebs_t), GFP_KERNEL | __GFP_ZERO, cpu_to_node(cpu));
    if (unlikely(!buffer)) {
        ERROR("Cannot allocate PEBS buffer on %d CPU", cpu);
        return VTSS_ERR_NOMEMORY;
    }
...
static void vtss_pebs_release_buffer(int cpu)
{
    if (pcb(cpu).pebs_ptr != NULL)
        kfree(pcb(cpu).pebs_ptr);
}
#endif
...
{
    if (hardcfg.family == VTSS_FAM_P6 && vtss_pebs_record_size) {
...
int vtss_pebs_init(void)
{
    ...
    on_each_cpu(vtss_pebs_on_each_cpu_func, NULL, 1);

    REPORT("PEBSv%d: record size: 0x%02lx, mask: 0x%02llx", vtss_pebs_version(), vtss_pebs_record_size, vtss_pebs_enable_mask);
