...
void vtss_task_map_reclaim_item(struct rcu_head *rp)
{
    ...
    DEBUG_TASKMAP("start, item = %p", item);
    if (atomic_read(&item->usage) == 0) {
        DEBUG_TASKMAP("usage 0, item = %p", item);
        if (item->dtor)
            item->dtor(item, NULL);
        item->dtor = NULL;
        vtss_kfree(item);
...
#ifdef VTSS_AUTOCONF_HLIST_NODE_ITER
...
int vtss_task_map_put_item(vtss_task_map_item_t *item)
{
    ...
    if ((item != NULL) && atomic_dec_and_test(&item->usage)) {
        ...
        if (atomic_read(&item->in_list)) {
            ...
            if (atomic_read(&item->in_list)) {
                DEBUG_TASKMAP("removing from the list");
        ...
        if (atomic_read(&item->usage) == 0) { /* do not remove this check! */
            DEBUG_TASKMAP("before call_rcu, item = %p", item);
            call_rcu(&item->rcu, vtss_task_map_reclaim_item);
            DEBUG_TASKMAP("after call_rcu");
            return 1;
        }
    }
 ...
 */
vtss_task_map_item_t *vtss_task_map_alloc(pid_t key, size_t size, vtss_task_map_func_t *dtor)
{
    vtss_task_map_item_t *item = NULL;

    if (atomic_read(&vtss_map_initialized) == 0)
        return NULL;
    item = vtss_kmalloc(sizeof(vtss_task_map_item_t) + size, GFP_ATOMIC);
    if (item != NULL) {
