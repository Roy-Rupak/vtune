...
int vtss_record_magic(struct vtss_transport_data *trnd)
{
    static unsigned int marker[2] = { UEC_MAGIC, UEC_MAGICVALUE };
    return vtss_transport_record_write(trnd, marker, sizeof(marker), NULL, 0);
}

int vtss_record_debug_info(struct vtss_transport_data *trnd, const char *message)
{
    int rc = 0;
    if (likely(message != NULL)) {
        ...
        rc = vtss_transport_record_write(trnd, &dbgrec, sizeof(dbgrec), (void*)message, msglen);
    }
    return rc;
}

int vtss_record_process_exec(struct vtss_transport_data *trnd, pid_t tid, pid_t pid, int cpu, const char *filename)
{
    ...
    return vtss_transport_record_write(trnd, &procrec, sizeof(procrec), (void*)filename, namelen);
}

int vtss_record_process_exit(struct vtss_transport_data *trnd, pid_t tid, pid_t pid, int cpu, const char *filename)
{
    size_t namelen = filename ? strlen(filename) + 1 : 0;
    prc_trace_record_t procrec;
    procrec.flagword = UEC_LEAF1 | UECL1_ACTIVITY | UECL1_CPUIDX | UECL1_USRLVLID | UECL1_CPUTSC | UECL1_REALTSC | UECL1_SYSTRACE;
...
int vtss_record_thread_create(struct vtss_transport_data *trnd, pid_t tid, pid_t pid, int cpu)
{
    ...
    return vtss_transport_record_write(trnd, &threc, sizeof(threc), NULL, 0);
}

int vtss_record_thread_stop(struct vtss_transport_data *trnd, pid_t tid, pid_t pid, int cpu)
{
    nth_trace_record_t threc;
    threc.flagword = UEC_LEAF1 | UECL1_ACTIVITY | UECL1_VRESIDX | UECL1_CPUIDX | UECL1_USRLVLID | UECL1_CPUTSC | UECL1_REALTSC;
...
int vtss_record_switch_from(struct vtss_transport_data *trnd, int cpu, int is_preempt)
{
    ...
    return vtss_transport_record_write(trnd, &ctxrec, sizeof(ctxrec.sysout), NULL, 0);
}

int vtss_record_switch_to(struct vtss_transport_data *trnd, pid_t tid, int cpu, void *ip)
{
    ...
    return vtss_transport_record_write(trnd, &ctxrec, sizeof(ctxrec.procina), NULL, 0);
}

#define VTSS_MAX_ACTIVE_CPUEVENTS VTSS_CFG_CHAIN_SIZE/10

int vtss_record_sample(struct vtss_transport_data *trnd, pid_t tid, int cpu, cpuevent_t *cpuevent_chain, void *ip)
{
    int i, j, rc = -EFAULT;
    unsigned long flags;
    ...
    if (ip != NULL) {
        ...
        rc = vtss_transport_record_write(trnd, &eventrec.sperec, sizeof(eventrec.sperec), scratch, (j+1)*sizeof(unsigned long long));
    } else {
        ...
        rc = vtss_transport_record_write(trnd, &eventrec.gperec, sizeof(eventrec.gperec), scratch, j*sizeof(unsigned long long));
    }
    local_irq_restore(flags);
    return rc;
}

int vtss_record_bts(struct vtss_transport_data *trnd, pid_t tid, int cpu, void *bts_buff, size_t bts_size)
{
    bts_trace_record_t btsrec;

    if (bts_size >= ((unsigned short)~0)-4) {
        ERROR("BTS buffer too big");
        return -1;
    }
    ...
    return vtss_transport_record_write(trnd, &btsrec, sizeof(bts_trace_record_t), bts_buff, bts_size);
}

int vtss_record_module(struct vtss_transport_data *trnd, int m32, unsigned long addr, unsigned long size, const char *pname, unsigned long pgoff, long long cputsc, long long realtsc)
{
    int rc = -EFAULT;
    ...
    if (modrec.type == UECSYSTRACE_MODULE_MAP32) {
        ...
        rc = vtss_transport_record_write(trnd, &modrec32, sizeof(modrec32), (void*)pname, modrec32.len);
        if (rc) ERROR("Failed to write map32: [0x%lx-0x%lx]: %s", addr, addr + size, pname);
        return rc;
    }
    rc = vtss_transport_record_write(trnd, &modrec, sizeof(modrec), (void*)pname, modrec.len);
    if (rc) ERROR("Failed to write map: [0x%lx-0x%lx]: %s", addr, addr + size, pname);
    return rc;
}

int vtss_record_configs(struct vtss_transport_data *trnd, int m32)
{
    int rc = 0;
    unsigned short size;
    ...
    /* generate forward compatibility format record (always) */
    {
        ...
        rc |= vtss_transport_record_write(trnd, &fcfrec, sizeof(fcfrec), (void*)fmtcfg, sizeof(fmtcfg));
    }
    ...
    /* generate collector configuration record */
    {
        ...
        rc |= vtss_transport_record_write(trnd, &colrec, sizeof(colrec), (void*)colname, sizeof(colname));
    }
    ...
    /* generate system configuration record */
    {
        ...
        rc |= vtss_transport_record_write(trnd, &sysrec, sizeof(sysrec), (void*)&syscfg, size);
    }
    ...
    /* generate hardware configuration record */
    {
        ...
        rc |= vtss_transport_record_write(trnd, &hcfrec, sizeof(hcfrec), (void*)&hardcfg, size);
    }
    ...
    /* generate Intel PT hardware configuration record */
    {
        ...
        rc |= vtss_transport_record_write(trnd, &ptcrec, sizeof(ptcrec), (void*)&iptcfg, sizeof(iptcfg));
    }
    /* generate time marker record */
    {
        ...
        timark.utc      = vtss_time_ts_to_ns(&now)/100ULL;
        rc |= vtss_transport_record_write(trnd, &timark, sizeof(timark), NULL, 0);
    }

    return rc;
}

int vtss_record_softcfg(struct vtss_transport_data *trnd, pid_t tid)
{
    int i, rc = 0;
    int evtsize;
    ...
    if (reqcfg.cpuevent_count_v1 > 0) {
        ...
        for (i = 0; i < reqcfg.cpuevent_count_v1; i++) {
            ...
            if (size + evtsize >= VTSS_SCRATCH_SIZE) {
                break;
            }
        ...
        rc |= vtss_transport_record_write(trnd, &scfrec, sizeof(scfrec), (void*)softcfg, size);
        local_irq_restore(flags);
    }
    return rc;
}

int vtss_record_probe(struct vtss_transport_data *trnd, int cpu, int fid)
{
    ...
    proberec.fid       = fid;
    return vtss_transport_record_write(trnd, &proberec, sizeof(proberec), NULL, 0);
}

int vtss_record_probe_all(int cpu, int fid)
{
    prb_trace_record_t proberec;
    proberec.flagword  = UEC_LEAF1 | UECL1_ACTIVITY | UECL1_VRESIDX | UECL1_CPUIDX | UECL1_CPUTSC | UECL1_USERTRACE;
    proberec.activity  = UECACT_PROBED;
    ...
    proberec.entry_cpu = proberec.cpuidx;
    proberec.fid       = fid;
    return vtss_transport_record_write_all(&proberec, sizeof(proberec), NULL, 0);
}

int vtss_record_thread_name(struct vtss_transport_data *trnd, pid_t tid, const char *taskname)
{
    ...
    return vtss_transport_record_write(trnd, &namerec, sizeof(namerec), (void*)taskname, namelen);
}
