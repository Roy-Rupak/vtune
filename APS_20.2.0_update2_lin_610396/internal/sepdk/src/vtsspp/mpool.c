...
#define VTSS_NR_CHUNKS_MIN 8
#define VTSS_NR_CHUNKS_MAX 128
...
static atomic_t g_vtss_memory_pool_used_size = ATOMIC_INIT(0);
static atomic_t g_vtss_memory_pool_max_used_size = ATOMIC_INIT(0);
...
static inline void vtss_memchunk_list_write_lock(void)
{
    ...
    {
        touch_nmi_watchdog();
        if (i == VTSS_WAIT_TIME) DEBUG_MPOOL("Cannot lock for write");
        i++;
    }
    i = 0;
    ...
    {
        touch_nmi_watchdog();
        if (i == VTSS_WAIT_TIME) DEBUG_MPOOL("Read lock is not free");
        i++;
    }
}
...
static inline void vtss_memchunk_list_read_lock(void)
{
    ...
    do
    {
        ...
        touch_nmi_watchdog();
        if (i == VTSS_WAIT_TIME) DEBUG_MPOOL("Cannot lock for read");
        i++;
    }
    while (1);
}
...
static atomic_t g_vtss_kernel_tasks_in_progress = ATOMIC_INIT(0);
...
void vtss_memory_pool_clear(void)
{
    ...
    list_for_each_safe(p, tmp, &g_memchunk_list)
    {
        ...
        {
            ERROR("Memory leaks detected, free_space = %zx, size = %zx", (size_t)atomic_read(&chunk->free_space), chunk->size);
            atomic_set(&chunk->free_space, chunk->size);
        }
#ifdef VTSS_MPOOL_SANITY_CHECK
        struct vtss_memory_block *temp = chunk->head;
    ...
    atomic_set(&g_vtss_memory_pool_used_size, 0);
    return;
}
...
struct vtss_memchunk *vtss_create_memchunk(gfp_t flags, unsigned int size)
{
    ...
    {
        DEBUG_MPOOL("Max number of memory chunks is reached");
        return NULL;
    }
    do
    {
        chunk = (struct vtss_memchunk*)__get_free_pages(GFP_KERNEL | __GFP_NOWARN, order);
        if (!chunk)
        {
            ERROR("Cannot allocate memory, order = %d, flags = %d", order, flags);
        }
    }
    while ((!chunk) && (--order) >= VTSS_ALLOC_ORDER_MIN);

    DEBUG_MPOOL("chunk = %p allocated, order = %x", chunk, order);
    ...
    atomic_set(&chunk->free_space, chunk->size);
    DEBUG_MPOOL("Chunk created: order = 0x%x, (PAGE_SIZE<<order) = %lx, sizeof(vtss_memchunk) = 0x%llx, sizeof(*chunk) = %llx, &chunk->memblocks = %llx, chunk->memblocks = %llx, chunk->size = %zu",
...
static void vtss_create_memchunk_work(struct work_struct *work)
{
    ...
    struct vtss_memchunk *chunk = NULL;
    DEBUG_MPOOL("Creating new chunc async");
    if (!my_work)
    {
        ERROR("Empty work");
        return;
    ...
    {
        vtss_kfree(my_work);
        ERROR("Mempool is not inited");
        atomic_dec(&g_vtss_kernel_tasks_in_progress);
        return;
    }
...
struct vtss_memchunk *vtss_create_memchunk_async(gfp_t flags, unsigned int size)
{
    ...
    if (in_atomic())
    {
        ...
        // create async
        DEBUG_MPOOL("The attempt to allocate memory in IRQs disabled mode. Start async");
        data.flags = flags;
        data.size = size;
        atomic_inc(&g_vtss_kernel_tasks_in_progress);
        if (atomic_read(&g_vtss_kernel_tasks_in_progress) > 1)
        {
            DEBUG_MPOOL("Cannot create async, g_vtss_kernel_tasks_in_progress = %d", atomic_read(&g_vtss_kernel_tasks_in_progress));
            atomic_dec(&g_vtss_kernel_tasks_in_progress);
            return NULL;
        }
        if (vtss_queue_work(-1, vtss_create_memchunk_work, &data, sizeof(data)))
        {
            DEBUG_MPOOL("Cannot create memchunk");
            atomic_dec(&g_vtss_kernel_tasks_in_progress);
        }
        // no need to wait memory creation. caller decides if it's reasonable
        //while (atomic_read(&g_vtss_kernel_tasks_in_progress));
        return NULL;
    }
    DEBUG_MPOOL("Creating chunk...");
    return vtss_create_memchunk(flags, size);
}

void vtss_destroy_memchunk(struct vtss_memchunk *chunk)
{
    DEBUG_MPOOL("chunk = %p, size = 0x%lx, deleting...", chunk, (unsigned long)chunk - (unsigned long)&chunk->memblocks + chunk->size);
...
void vtss_destroy_memchunk_async(struct vtss_memchunk *chunk)
{
    if (in_atomic())
    {
        DEBUG_MPOOL("The attempt to deallocate memory in IRQs disabled mode. Start async");
        atomic_inc(&g_vtss_kernel_tasks_in_progress);
        if (vtss_queue_work(-1, vtss_destroy_memchunk_work, &chunk, sizeof(chunk)))
        {
            DEBUG_MPOOL("Failed create work");
            atomic_dec(&g_vtss_kernel_tasks_in_progress);
        }
        return;
    }
...
static void vtss_delete_chunk_list(void)
{
    ...
    list_for_each_safe(p, tmp, &g_memchunk_list)
    {
        ...
        DEBUG_MPOOL("Chunk %p destroying...", chunk);
        vtss_destroy_memchunk(chunk);
        DEBUG_MPOOL("done");
    }
...
int vtss_memory_pool_init(void)
{
    ...
    if (nr_chunks > VTSS_NR_CHUNKS_MAX) nr_chunks = VTSS_NR_CHUNKS_MAX;
    if (nr_chunks < VTSS_NR_CHUNKS_MIN) nr_chunks = VTSS_NR_CHUNKS_MIN;
    DEBUG_MPOOL("prealloc_buf_size = %llx, num_present_cpus = %d", prealloc_buf_size, num_present_cpus());
    ...
    for (i = 0; i < nr_chunks; i++)
    {
        ...
        if (!chunk)
        {
            ERROR("Not enough memory for memchunk[%d]", i);
            return -1;
        }
    }
    atomic_inc(&g_vtss_mempool_init);

    REPORT("Reserved %lldMB for internal memory pool", (nr_chunks*prealloc_buf_size)>>20);
...
void vtss_memory_pool_fini(void)
{
    ...
    DEBUG_MPOOL("Ok. No active kernel tasks.");
    i = 0;
    vtss_delete_chunk_list();
    DEBUG_MPOOL("Empty memory pool");

    REPORT("Maximum used %dMB from internal memory pool", atomic_read(&g_vtss_memory_pool_max_used_size)>>20);
...
static struct vtss_memory_block *vtss_find_free_block(struct vtss_memchunk *chunk, size_t size)
{
    ...
    {
        DEBUG_MPOOL("finding tail, temp = %p", temp);
        temp = temp->prev;
    ...
    if (temp)
    {
        ...
        DEBUG_MPOOL("(unsigned long)temp + sizeof(*temp) + temp->size;%lx", start_addr);
    ...
    if (!temp)
    {
        ...
        DEBUG_MPOOL("Chunk is empty. Return %p", block);
        if (!block)
        {
    ...
    if (end_addr - start_addr >= size + sizeof(struct vtss_memory_block))
    {
        DEBUG_MPOOL("Return tail block that was deallocated in different thread");
        block = (struct vtss_memory_block*)start_addr;
...
unsigned long vtss_get_free_block(gfp_t gfp_mask, size_t size)
{
    ...
    list_for_each_safe(p, tmp, &g_memchunk_list)
    {
        ...
        if (block_addr > 0)
        {
            ...
            {
                DEBUG_MPOOL("ERROR: chunk = %p, &chunk->memblocks = %lx, chunk->size = %zx, block_addr = %lx, size = %lx, memblock+size = %lx", chunk, (unsigned long)&chunk->memblocks, chunk->size, block_addr, size, (unsigned long)(&chunk->memblocks) + chunk->size);
    ...
    atomic_add(size, &g_vtss_memory_pool_used_size);
    {
        int used = atomic_read(&g_vtss_memory_pool_used_size);
        int max_used = atomic_read(&g_vtss_memory_pool_max_used_size);
        if (max_used < used) atomic_cmpxchg(&g_vtss_memory_pool_max_used_size, max_used, used);
    }

    DEBUG_MPOOL("getting block size = %zu, found block_addr = %lx", size, block_addr);
    return block_addr;
}
...
void vtss_free_block(unsigned long block_addr, size_t size)
{
    ...
    DEBUG_MPOOL("free addr = %lx, block_addr = %lx", addr, block_addr);
    vtss_memchunk_list_read_lock();
    list_for_each_safe(p, tmp, &g_memchunk_list)
    {
        struct vtss_memchunk *chunk = list_entry(p, struct vtss_memchunk, list);
        ...
        if ((unsigned long)&chunk->memblocks[0] <= addr && addr < (unsigned long)chunk + sizeof(struct vtss_memchunk) + chunk->size)
        {
            ...
            do
            {
                DEBUG_MPOOL("Block will be marked as free: %p, size = %zx", block, block->size);
                ...
                {
                    DEBUG_MPOOL("Removing wrong size");
                }
            ...
            DEBUG_MPOOL("removed_size = %zx", removed_size);
            atomic_add(removed_size, &chunk->free_space);
            break;
        }
    }
    vtss_memchunk_list_read_unlock();
    atomic_sub(size, &g_vtss_memory_pool_used_size);
    return;
}
...
static unsigned long vtss_try_get_free_block(gfp_t flags, size_t size, int cnt)
{
    ...
    while (!block && (cnt--) > 0)
    {
        ...
        DEBUG_MPOOL("Cannot get block, try again");
    }
...
void *vtss_kmalloc_internal(size_t size, gfp_t flags)
{
    ...
    if (!block)
    {
        DEBUG_MPOOL("Attempt to create async, flags = %x\n", (int)flags);
