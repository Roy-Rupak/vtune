...
size_t vtss_user_vm_read(struct user_vm_accessor *acc, void *from, void *to, size_t size)
{
    ...
    do {
...
#endif
#ifdef VTSS_AUTOCONF_ACCESS_OK_TWO_ARGS
        ...
        rc = __copy_from_user_inatomic(to, acc->m_maddr + offset, cpsize);
#else
        rc = __copy_from_user_inatomic(to, (void*)addr, cpsize);
#endif
        if (rc) break;
...
user_vm_accessor_t *vtss_user_vm_accessor_init(cycles_t limit)
{
    ...
    acc = vtss_kmalloc(sizeof(user_vm_accessor_t), GFP_ATOMIC);
    if (acc != NULL) {
