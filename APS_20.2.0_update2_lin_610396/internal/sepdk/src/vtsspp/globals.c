...
static void vtss_hardcfg_init(void)
{
    ...
    if (vtss_ktime_equal(KTIME_MONOTONIC_RES, KTIME_LOW_RES)) {
        WARNING("An accuracy of kernel timer is not enough. Switch to TSC.");
    ...
    if (eax1.split.family == VTSS_FAM_P4)
        hardcfg.family += eax1.split.family_ext;
    hardcfg.model = eax1.split.model;
    if (eax1.split.family == VTSS_FAM_P6 || eax1.split.family == VTSS_FAM_P4)
    ...
    if (hardcfg.cpu_no == 1) {
        ...
        if (eax1.split.family == VTSS_FAM_P6) {
    ...
    REPORT("CPU freq: %lldKHz, timer freq: %lldKHz", hardcfg.cpu_freq/1000, hardcfg.timer_freq/1000);
...
static void vtss_iptcfg_init(void)
{
    ...
    {
        iptcfg.version = 0;
        iptcfg.fratio = extract_bits((unsigned)vtss_read_msr(0xce), 8, 8);
...
int vtss_globals_init(void)
{
    ...
    for_each_possible_cpu(cpu) {
        ...
        ppcb->scratch_ptr = kmalloc_node(VTSS_SCRATCH_SIZE, GFP_KERNEL, cpu_to_node(cpu));
        if (ppcb->scratch_ptr == NULL)
            goto fail;
    }
    vtss_syscfg_init();
