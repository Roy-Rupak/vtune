...
cpumask_t vtss_procfs_cpumask = CPU_MASK_NONE;
int vtss_procfs_defsav = 0;
...
static void vtss_procfs_set_pce_cpu(void *arg)
{
    int to_val = (arg != 0);
    unsigned long cr4_val = 0;

    cr4_val = vtss_read_cr4();
    if (to_val)
    {
...
static ssize_t vtss_procfs_ctrl_write(struct file *file, const char __user * buf, size_t count, loff_t * ppos)
{
    ...
    while (buf_size > 0) {
        ...
        case 'V': { /* VXXXXX.XXXXX client version */
                ...
                DEBUG_PROCFS("VERSION: %u.%u", vtss_client_major_ver, vtss_client_minor_ver);
                break;
        }
        ...
        case 'I': { /* I<flags> - Initialize */
                ...
                DEBUG_PROCFS("INIT: flags=0x%lx", flags);
                if (vtss_cmd_start() != 0)
                {
        ...
        case 'E': { /* E<size>=... - configuration request */
                ...
                DEBUG_PROCFS("EVENTS: size=%lu", size);
                vtss_reqcfg_init();
                if (chr == '=' && size <= buf_size) {
                    ...
                    while (size != 0) {
                        ...
                        case VTSS_CFGREQ_CPUEVENT_V1:
                            ...
                            if (reqcfg.cpuevent_count_v1 < VTSS_CFG_CHAIN_SIZE) {
                                ...
                                if (namespace_size + cpuevent_currreq->name_len + cpuevent_currreq->desc_len < VTSS_CFG_SPACE_SIZE * 16) {
                                    ...
                                    DEBUG_PROCFS("event[%02d]: %s", reqcfg.cpuevent_count_v1, &reqcfg.cpuevent_namespace_v1[namespace_size]);
                        ...
                        case VTSS_CFGREQ_STK:
                            ...
                            DEBUG_PROCFS("VTSS_CFGREQ_STK: stk_sz=0x%lx, stk_pg_sz=0x%lx", reqcfg.stk_sz[stk_req.stktype], reqcfg.stk_pg_sz[stk_req.stktype]);
                        ...
                        case VTSS_CFGREQ_IPT:
                            ...
                            DEBUG_PROCFS("VTSS_CFGREQ_IPT: reqcfg.ipt_cfg.mode=0x%x", reqcfg.ipt_cfg.mode);
                    ...
                    if (!vtss_ipt_support_level())
                    {
                        ...
                        if (reqcfg.trace_cfg.trace_flags & VTSS_CFGTRACE_IPT)
                        {
                            ...
                            {
                                DEBUG_PROCFS("VTSS_CFGREQ_IPT: remove stacks, ipt_cfg.mode=0x%x", reqcfg.ipt_cfg.mode);
        ...
        case 'W': /* W - Watchdog */
            ...
            if (get_user(chr, buf))
                return -EFAULT;
            DEBUG_PROCFS("WATCHDOG: cmd=%c", chr);
            buf += sizeof(char);
            buf_size -= sizeof(char);
            if (chr == '0') {
        ...
        case 'C':
            ...
            DEBUG_PROCFS("SETPCE: cmd=%c", chr);
            buf += sizeof(char);
            buf_size -= sizeof(char);
            if (chr == '1') {
                on_each_cpu(vtss_procfs_set_pce_cpu, (void*)1, 1);
            }
            else {
                on_each_cpu(vtss_procfs_set_pce_cpu, (void*)0, 1);
            }
            break;
        case ' ':
        case '\n':
...
static VTSS_DEFINE_SPINLOCK(vtss_procfs_ctrl_list_lock);
...
{
    unsigned long flags;
    struct vtss_procfs_ctrl_data *ctld = vtss_kmalloc(sizeof(struct vtss_procfs_ctrl_data) + size, GFP_ATOMIC);
    ...
    if (size) {
        ...
        DEBUG_PROCFS("msg=%s, size=%d", (char*)msg, (int)size);
    } else {
        DEBUG_PROCFS("EOF");
    }
    ctld->size = size;
...
static ssize_t vtss_procfs_ctrl_read(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
    ...
    DEBUG_PROCFS("file=0x%p, msg=%s, %zu of %zu", file, ctld->buf, ctld->size, size);
    if (rsize > size) {
...
static int vtss_procfs_ctrl_close(struct inode *inode, struct file *file)
{
    if (atomic_dec_and_test(&vtss_procfs_attached)) {
        ...
        cpumask_copy(&vtss_procfs_cpumask, cpu_present_mask);
        vtss_procfs_defsav = 0;
    }
    vtss_cmd_close();
...
static const struct vtss_procfs_ops vtss_procfs_ctrl_fops = {
    VTSS_PROCFS_DEFAULT_OPS
    .vtss_procfs_read    = vtss_procfs_ctrl_read,
    .vtss_procfs_write   = vtss_procfs_ctrl_write,
    .vtss_procfs_open    = vtss_procfs_ctrl_open,
    .vtss_procfs_release = vtss_procfs_ctrl_close,
    .vtss_procfs_poll    = vtss_procfs_ctrl_poll,
};
...
static const struct vtss_procfs_ops vtss_procfs_debug_fops = {
    VTSS_PROCFS_DEFAULT_OPS
    .vtss_procfs_open    = vtss_procfs_debug_open,
...
static ssize_t vtss_procfs_cpumask_read(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
    ...
    if (*ppos == 0) {
        char *page = vtss_kmalloc(PAGE_SIZE, GFP_KERNEL);
        ...
        rc = cpulist_scnprintf(page, PAGE_SIZE-2, &vtss_procfs_cpumask);
#else
        rc = scnprintf(page, PAGE_SIZE-2, "%*pbl", cpumask_pr_args(&vtss_procfs_cpumask));
#endif
        page[rc++] = '\n';
        page[rc]   = '\0';
        *ppos += rc;
...
static ssize_t vtss_procfs_cpumask_write(struct file *file, const char __user * buf, size_t count, loff_t * ppos)
{
    ...
    if (!cpumask_parselist_user(buf, count, new_value)) {
        cpumask_and(&vtss_procfs_cpumask, new_value, cpu_present_mask);
        rc = count;
    }
...
static const struct vtss_procfs_ops vtss_procfs_cpumask_fops = {
    VTSS_PROCFS_DEFAULT_OPS
...
static ssize_t vtss_procfs_defsav_read(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
    ...
    if (*ppos == 0) {
        ...
        rc = snprintf(buff, sizeof(buff)-2, "%d", vtss_procfs_defsav);
        rc = (rc < 0) ? 0 : rc;
        buff[rc++] = '\n';
        buff[rc]   = '\0';
        *ppos += rc;
...
static ssize_t vtss_procfs_defsav_write(struct file *file, const char __user * buf, size_t count, loff_t * ppos)
{
    ...
    vtss_procfs_defsav = (int)(val < 1000000)    ? 1000000    : val;
    vtss_procfs_defsav = (int)(val > 2000000000) ? 2000000000 : val;
    return count;
}
...
static const struct vtss_procfs_ops vtss_procfs_defsav_fops = {
    VTSS_PROCFS_DEFAULT_OPS
    .vtss_procfs_read    = vtss_procfs_defsav_read,
    .vtss_procfs_write   = vtss_procfs_defsav_write,
    .vtss_procfs_open    = vtss_procfs_defsav_open,
    .vtss_procfs_release = vtss_procfs_defsav_close,
};
...
static const struct vtss_procfs_ops vtss_procfs_targets_fops = {
    VTSS_PROCFS_DEFAULT_OPS
...
static ssize_t vtss_procfs_timesrc_write(struct file *file, const char __user * buf, size_t count, loff_t * ppos)
{
    ...
    if (!strncmp(val, "tsc", 3)) {
        if (check_tsc_unstable()) {
            WARNING("TSC time source is unstable");
        }
...
static const struct vtss_procfs_ops vtss_procfs_timesrc_fops = {
    VTSS_PROCFS_DEFAULT_OPS
...
static const struct vtss_procfs_ops vtss_procfs_timelimit_fops = {
    VTSS_PROCFS_DEFAULT_OPS
    .vtss_procfs_read    = vtss_procfs_timelimit_read,
    .vtss_procfs_write   = vtss_procfs_timelimit_write,
    .vtss_procfs_open    = vtss_procfs_timelimit_open,
    .vtss_procfs_release = vtss_procfs_timelimit_close,
};
...
#endif
        if (vtss_procfs_root != NULL) {
            vtss_procfs_set_user(vtss_procfs_root, uid, gid);
        }
    }
...
static int vtss_procfs_create_entry(const char *name, const struct vtss_procfs_ops *fops)
{
    ...
    vtss_procfs_set_user(pde, uid, gid);
    return 0;
}
